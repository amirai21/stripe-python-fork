...
import ai21
from ai21 import six
from ai21.six.moves.urllib.parse import parse_qsl, quote_plus
...
def _test_or_live_environment():
    if ai21.api_key is None:
        return
    match = re.match(r"sk_(live|test)_", ai21.api_key)
    if match is None:
        return
...
def get_object_classes():
    ...
    from ai21.object_classes import OBJECT_CLASSES
...
def convert_to_stripe_object(
    ...
    if isinstance(resp, ai21.stripe_response.StripeResponse):
    ...
    if isinstance(resp, list):
        ...
        resp, ai21.stripe_object.StripeObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, six.string_types):
            klass = get_object_classes().get(
                klass_name, ai21.stripe_object.StripeObject
            )
        else:
            klass = ai21.stripe_object.StripeObject
