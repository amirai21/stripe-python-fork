...
import ai21
from ai21 import error, util, six
from ai21.request_metrics import RequestMetrics
...
try:
    from ai21.six.moves import urllib
except ImportError:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                ...
                "questions, please contact support@ai21.com. (HINT: running "
...
from ai21.six.moves.urllib.parse import urlparse
...
class HTTPClient(object):
    ...
    def _should_retry(self, response, api_connection_error, num_retries):
        ...
        if rheaders is not None and "ai21-should-retry" in rheaders:
            if rheaders["ai21-should-retry"] == "false":
                return False
            if rheaders["ai21-should-retry"] == "true":
                return True
        ...
        #
        # Note that we expect the ai21-should-retry header to be false
    ...
    def _max_network_retries(self):
        from ai21 import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if ai21.enable_telemetry and last_request_metrics:
            telemetry = {
    ...
    def _record_request_metrics(self, response, request_start):
        _, _, rheaders = response
        if "Request-Id" in rheaders and ai21.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = ai21.ca_bundle_path
        else:
            kwargs["verify"] = False
    ...
    def __init__(self, verify_ssl_certs=True, proxy=None, deadline=55):
        ...
        if proxy:
            raise ValueError(
                ...
                "Set ai21.default_http_client to either RequestsClient, "
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            ...
            msg = (
                ...
                "Please let us know at support@ai21.com."
            )
        else:
            msg = (
                "Unexpected error communicating with Stripe. If this "
                "problem persists, let us know at support@ai21.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.CAINFO, ai21.ca_bundle_path)
        else:
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = (
            "Unexpected error communicating with Stripe. "
            "If this problem persists, let us know at support@ai21.com."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
