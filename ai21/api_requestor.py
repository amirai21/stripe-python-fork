...
import ai21
from ai21 import error, oauth_error, http_client, version, util, six
from ai21.multipart_data_generator import MultipartDataGenerator
from ai21.six.moves.urllib.parse import urlencode, urlsplit, urlunsplit
from ai21.stripe_response import StripeResponse, StripeStreamResponse
...
class APIRequestor(object):
    def __init__(
        ...
        self.api_base = api_base or ai21.api_base
        self.api_key = key
        self.api_version = api_version or ai21.api_version
        self.stripe_account = account

        self._default_proxy = None

        from ai21 import verify_ssl_certs as verify
        from ai21 import proxy

        if client:
            self._client = client
        elif ai21.default_http_client:
            self._client = ai21.default_http_client
            if proxy != self._default_proxy:
                warnings.warn(
                    "ai21.proxy was updated after sending a "
                    "request - this is a no-op. To use a different proxy, "
                    "set ai21.default_http_client to a new client "
                    "configured with the proxy."
                )
        else:
            # If the ai21.default_http_client has not been set by the user
            # yet, we'll set it here. This way, we aren't creating a new
            # HttpClient for every request.
            ai21.default_http_client = http_client.new_default_http_client(
    ...
    def request_headers(self, api_key, method):
        ...
        if ai21.app_info:
            user_agent += " " + self.format_app_info(ai21.app_info)
    ...
    def request_raw(
        ...
        if my_api_key is None:
            raise error.AuthenticationError(
                ...
                '"ai21.api_key = <API-KEY>"). You can generate API keys '
                "from the Stripe web interface.  See https://stripe.com/api "
                "for details, or email support@ai21.com if you have any "
                "questions."
            )
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "Stripe bindings.  Please contact support@ai21.com for "
                "assistance." % (method,)
            )
